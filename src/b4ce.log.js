(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        root.B4ce.Log = factory(root._, root.B4ce.Log.Util, root.B4ce.Log.Channel);
    }
}(this, function (_, Util, Channel) {
'use strict';
...
//            console.log('Log(): categoryLevel:', this.categoryLevel);
    ...
    if (!options.channels) {
        ...
        if (Channel.Console) {
            options.channels.console = new Channel.Console({
                log: this
            });
        }
        if (Channel.Alert) {
            options.channels.alert = new Channel.Alert({
                log: this
            });
        }
    }
            ...
            for (curLvl in this.levels) {
                if (this.levels.hasOwnProperty(curLvl)) {
                    ...
                    if (this[curMeth] !== undefined) {
                        ...
                        this[curMeth] = Util.createMethod(curCat, curLvl);
                    }
                }
            }
        }
